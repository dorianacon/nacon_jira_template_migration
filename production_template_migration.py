...
if not st.session_state.connected:
    st.header("Jira instance connection")
    ...
    username = st.text_input("Email / Username", value=default_email)
    token_or_password = st.text_input("API token / Password", value=default_token, type="password")
    cloud_checkbox = st.checkbox("Atlassian Cloud (email + API token)", value=True)

    if st.button("Connect"):
        st.session_state.last_error = None
        if not jira_url or not username or not token_or_password:
            st.session_state.last_error = "You must fill Jira URL, Email / Username and API token / Password fields"
            st.session_state.connected = False
            st.session_state.jira_client = None
        else:
            try:
                client = create_jira_connection(
                    jira_url.strip(),
                    username.strip(),
                    token_or_password.strip(),
                    cloud=cloud_checkbox
                )
                st.session_state.jira_client = client
                st.session_state.connected = True
                st.session_state.login_attempt = True
                st.session_state.username = username.strip()
                st.session_state.token = token_or_password.strip()
